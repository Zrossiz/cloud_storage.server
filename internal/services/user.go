...
func CreateUser(w http.ResponseWriter, r *http.Request, db *gorm.DB, redis *redis.Client) {
...
func LoginUser(w http.ResponseWriter, r *http.Request, db *gorm.DB, redis *redis.Client) {
	body, err := io.ReadAll(r.Body)
	if err != nil {
		response.SendError(w, http.StatusInternalServerError, "Error reading request body")
		return
